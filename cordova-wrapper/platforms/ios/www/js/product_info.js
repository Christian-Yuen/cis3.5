// product_info.js - Firebase v8 (compat) version - FIXED
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Configure Firestore for Cordova (helps with CORS)
    if (db.settings) {
        try {
            db.settings({
                cacheSizeBytes: firebase.firestore.CACHE_SIZE_UNLIMITED,
                experimentalForceLongPolling: true // Helps with CORS in Cordova
            });
    ...
    auth.onAuthStateChanged(function(user) {
        ...
        db.collection('users').doc(currentUserId).get()
            ...
            .catch(function(error) {
                ...
                // Still try to load product info
                loadProductInfo(itemId);
            });
    });
    ...
    function createContainersIfNeeded() {
        ...
        if (!document.getElementById('info')) {
            ...
            // Insert it at the beginning of body
    ...
    function loadProductImage(data) {
        ...
        // Add a fallback if the image doesn't load
        setTimeout(function() {
