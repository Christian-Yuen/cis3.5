// profile.js - Firebase v8 (compat) with CORS fix
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Configure Firestore for Cordova (CRITICAL FOR CORS)
    if (db.settings) {
        try {
            db.settings({
                cacheSizeBytes: firebase.firestore.CACHE_SIZE_UNLIMITED,
                experimentalForceLongPolling: true,  // Fixes CORS in Cordova
                merge: true
            });
    ...
    auth.onAuthStateChanged(function(user) {
        ...
        loadUserProfile(currentUserId)
            .then(function() {
                // Then load donations and reservations
    ...
    async function getImageUrl(imageUrl) {
        ...
        try {
            ...
            // Return as-is (might be a direct URL)
            return imageUrl;
